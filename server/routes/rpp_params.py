...
RPP_PARAM_SCHEMA: dict[str, dict] = {
    ...
    "segment_timeout_heading_tolerance_deg": {
        ...
        "default": 3.0,
        "group": "Tracking Profile",
        "description": "Heading tolerance after pivot timeout (deg). Pivot-release triple is 2/3/3 with segment_heading_tolerance_deg and segment_pivot_release_max_deg",
        "min": 0.1,
        "max": 45.0,
    },
    "segment_pivot_release_max_deg": {
        ...
        "default": 3.0,
        "group": "Tracking Profile",
        "description": "Absolute heading-error ceiling for pivot release (deg). Pivot-release triple is 2/3/3",
        "min": 0.1,
        "max": 45.0,
    },
    ...
    "stop_latch_enabled": {
        ...
        "default": False,
        "group": "Stop Latch",
        "description": "FIELD A/B 2026-08-04: enforce drive cmd in {0} U [min_actuatable, vmax] — no dead-band creep. Approach holds >= 0.10 m/s, hard-zero position latch within capture distance, nudge if resting short. Fixes strand/creep-through/surge at stop points (stg_c8e867b9 / stg_6de3fcce)",
    },
    "stop_latch_min_actuatable_m_s": {
        "type": "float",
        "default": 0.10,
        "group": "Stop Latch",
        "description": "PX4 RO_SPEED_TH dead-band edge: minimum command that actually turns the wheels",
        "min": 0.0,
        "max": 0.3,
    },
    "stop_latch_capture_dist_m": {
        "type": "float",
        "default": 0.02,
        "group": "Stop Latch",
        "description": "Distance to the stop point at which the command cuts to hard zero (position capture). Coast from 0.10 m/s at RO_DECEL_LIM 0.3 is ~1.7 cm",
        "min": 0.005,
        "max": 0.2,
    },
    "stop_latch_release_dist_m": {
        "type": "float",
        "default": 0.20,
        "group": "Stop Latch",
        "description": "A drive demand with the stop reference farther than this releases the latch (next segment/run relaunch)",
        "min": 0.05,
        "max": 1.0,
    },
