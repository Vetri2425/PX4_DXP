...
RPP_PARAM_SCHEMA: dict[str, dict] = {
    ...
    "align_recenter_arm_m": {
        ...
        "default": 0.08,
        "group": "Tracking Profile",
        "description": "Run-boundary CORNER_ALIGN recenter hysteresis (m): arm the position-recovery hold only once drift from the boundary point exceeds this. Paired with align_recenter_release_m so the recenter does not toggle on/off every cycle the pivot's own rotation nudges position past a single fixed threshold. Only applies when segment_entry_pivot_recenter=True.",
        "min": 0.0,
        "max": 0.30,
    },
    "align_recenter_release_m": {
        ...
        "default": 0.05,
        "group": "Tracking Profile",
        "description": "Run-boundary CORNER_ALIGN recenter hysteresis (m): release the position-recovery hold back to pivot-only once drift drops under this. Must be <= align_recenter_arm_m. Only applies when segment_entry_pivot_recenter=True.",
        "min": 0.0,
        "max": 0.30,
    },
