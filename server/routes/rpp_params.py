...
RPP_PARAM_SCHEMA: dict[str, dict] = {
    ...
    "segment_slowdown_dist": {
        ...
        "default": 1.00,
        "group": "Tracking Profile",
        "description": "Hard-corner stop approach distance before a non-final corner (m)",
        "min": 0.0,
        "max": 5.0,
    },
    "segment_min_corner_speed": {
        ...
        "default": 0.08,
        "group": "Tracking Profile",
        "description": "Pivot rollout speed during segment corner alignment (m/s)",
        "min": 0.0,
        "max": 1.0,
    },
    ...
    "segment_boundary_capture_radius_m": {
        ...
        "default": 0.50,
        "group": "Tracking Profile",
        "description": "Radius from a smooth-run pivot boundary (e.g. entry transit → first MARK point) where the approach switches to a fixed-bearing decel straight at the goal — a steady bearing PX4 tracks down to a clean sub-2cm stop, avoiding the pure-pursuit lookahead flip and PX4's spot-turn speed-hold (m). 0 disables.",
        "min": 0.0,
        "max": 1.5,
    },
    "segment_boundary_corner_handoff_m": {
        ...
        "default": 0.05,
        "group": "Tracking Profile",
        "description": "Inside the capture radius, distance below which the smooth-run final settle hands off to the segment corner-hold (tangent servo + brake to <2cm); above it the fixed-bearing decel approach runs (m). Keep below capture radius, above xy_goal_tolerance.",
        "min": 0.0,
        "max": 0.5,
    },
    "segment_endpoint_approach_speed": {
        ...
        "default": 0.03,
        "group": "Tracking Profile",
        "description": "Low stop-approach floor for final endpoints and hard stop-and-pivot corners (m/s)",
        "min": 0.0,
        "max": 0.5,
    },
    ...
    "align_recenter_arm_m": {
        "type": "float",
        "default": 0.08,
        "group": "Tracking Profile",
        "description": "CORNER_ALIGN recenter hysteresis (m), shared by run-boundary (gated by segment_entry_pivot_recenter=True) and intra-run segment corners (INTRA_RUN_CORNER, always on): arm the position-recovery hold only once drift from the corner point exceeds this. Paired with align_recenter_release_m so the recenter does not toggle on/off every cycle the pivot's own rotation nudges position past a single fixed threshold.",
        "min": 0.0,
        "max": 0.30,
    },
    "align_recenter_release_m": {
        ...
        "default": 0.05,
        "group": "Tracking Profile",
        "description": "CORNER_ALIGN recenter hysteresis (m), shared by run-boundary (gated by segment_entry_pivot_recenter=True) and intra-run segment corners (INTRA_RUN_CORNER, always on): release the position-recovery hold back to pivot-only once drift drops under this. Must be <= align_recenter_arm_m.",
        "min": 0.0,
        "max": 0.30,
    },
    "segment_entry_true_stop_dist_m": {
        ...
        "default": 0.10,
        "group": "Tracking Profile",
        "description": "Distance from a hard boundary stop point (entry or run-boundary, any profile) within which the terminal approach brakes straight to a true stop (segment _publish_zero parity) instead of holding a capture/hold setpoint that drives the coasting rover past the point. 0 disables (pure capture, smooth profile only). Keep >= corner_position_tolerance_m.",
        "min": 0.0,
        "max": 0.5,
    },
    "segment_entry_stop_speed_m_s": {
        ...
        "default": 0.03,
        "group": "Tracking Profile",
        "description": "Parked-cert speed gate (m/s) for ALL hard-boundary stops (entry and run-boundary, segment or smooth). Every hard-boundary stop runs the same active-brake true-stop machinery and certifies on THIS tighter gate instead of segment_stop_speed_threshold (0.08), which certified while the rover still crept 5-7 cm/s and the pivot walked past the boundary point. 0.03 = the at-rest definition, just above this rover's 0.5-2 cm/s noise floor and reachable because the rover is actively braked through it. Tighten to 0.02 if bags still show walk-past.",
        "min": 0.005,
        "max": 0.08,
    },
