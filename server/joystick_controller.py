...
Ported and hardened from `feat/entry-pivot-recenter:server/joystick_controller.py`
per docs/Architecture/JOYSTICK_CONTROLLER_PLAN.md §4.2. Two deliberate
deviations from the reference:

  1. Ownership claim uses `ControlArbiter.begin_joystick_acquire()` — a short,
     lock-scoped claim — instead of holding the arbiter lock across the whole
     acquire() body (FCU checks, neutral pre-stream, MANUAL mode-switch
     confirm). This removes the need for the reference's contextvars
     re-entry shim (plan §7.4).
  2. `emergency_neutralize()` now also clears arbiter ownership. The
     reference left the arbiter owner unchanged on e-stop, which would strand
     it at JOYSTICK_ACTIVE forever and block every future mission start.
"""
...
from control_arbiter import ControlArbiter, get_control_arbiter
...
class JoystickController:
    ...
    async def acquire(self, sid: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        await self._arbiter.begin_joystick_acquire(self._offboard_ctrl)
    ...
    def _stop_watchdog(self) -> None:
        ...
        try:
            ...
            current = None  # no running loop — safe to cancel unconditionally
