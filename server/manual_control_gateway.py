...
Ported from `feat/entry-pivot-recenter:server/manual_control_gateway.py`
...
class ManualControlGateway:
    ...
    The worker uses a thread instead of the Socket.IO asyncio loop so the
    last nonzero command cannot be repeated forever when command handling
    stalls. Keep exactly this shape — it is the innermost safety layer
    (plan §4.1).
    """
