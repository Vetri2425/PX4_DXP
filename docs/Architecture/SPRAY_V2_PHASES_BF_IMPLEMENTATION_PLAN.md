...
| **B** — RTK/GPS gate (§7.6) | Subscribe `/mavros/gpsstatus/gps1/raw`, threshold + asymmetric hysteresis | ✅ **DONE @`5947dd6`** — `_gps_cb` + gate in `_auto_safety_status` (AUTO only; manual test unaffected); params `spray_require_rtk_fix`/`spray_min_fix_type`(6)/`gps_fix_timeout_s`(2.0)/`gps_recover_hold_s`(1.0); instant-drop / slow-recover hysteresis; `gps_fix_ok`/`gps_fix_name` now real. 8 tests. | ✅ **DONE — bench-verified (live reads fix_type 3→"3D", ok=False); field-unverified** |
