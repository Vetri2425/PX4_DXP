...
| **B** — RTK/GPS gate (§7.6) | Subscribe `/mavros/gpsstatus/gps1/raw`, threshold + asymmetric hysteresis | ✅ **DONE @`5947dd6`**, hardened 2026-08-20 — `_gps_cb` + gate in `_auto_safety_status` (AUTO only; manual test unaffected); params `spray_require_rtk_fix`/`spray_min_fix_type`(6)/`gps_fix_timeout_s`(0.5)/`gps_recover_hold_s`(1.0); instant-drop / slow-recover hysteresis; `gps_fix_ok`/`gps_fix_name` now real. | ✅ **DONE — bench-verified (live reads fix_type 3→"3D", ok=False); field dropout still required** |
| **B0** — *(new, not in plan)* session-config transport | Implied by §3 but never given its own phase | ✅ **DONE @`0545fa1`** — node subscribes `/spray/session_config` (RELIABLE+TRANSIENT_LOCAL, fail-static, keeps last-known-good); rover-server publishes on load + explicit cleared on clear; `PathPlanRequest.spray_mode` staged; `_publish_status` reports real mode + `mode_state`. Anti-drift round-trip test (server builder → node parser). | ✅ **DONE — bench-verified, field-unverified** |
| **C** — Dash | `spray_modes.py` dash arc-length toggle math | ✅ **DONE @`73c3b3b`** — `spray_modes.DashMeter`: cumulative-arclength metering (continuous-across-mission, operator-locked), monotonic-`s` + jump-reject/recover, solenoid-lead on the ideal grid. Corner deferral free via the pivot-state gate. `_make_spray_decision` mode-aware. 9 pure + 5 wiring tests. | ✅ **DONE — bench-verified (live node switches, meters); field-unverified** |
| **D** — Point | Node FSM + coordinate ingest + **planner-side hold contract** | ✅ **BOTH HALVES DONE.** Node @`9218de4`: `PointMeter` FSM (arrive/settle/dwell/OFF-confirm-before-advance/watchdog-skip), routing, mode-appropriate config gate, Rev 4 §7.3 pivot-gate exemption, `mode_state`; builder emits point. **Planner @`25850c9`: RPP point-hold A/B** — gated overlay `_point_hold_tick` brakes+holds at each must-hit waypoint for `point_hold_s` (reuses corner-brake/stop primitives), **default OFF = byte-for-byte frozen**. must_hit already flows on `/path`. 9+3+2 node + 5 RPP-hold tests. | ✅ **DONE (default OFF) — bench-verified, field-unverified.** Enable `point_hold_enabled:=true` + set `point_hold_s` ≥ spray settle+dwell+margin for the named A/B run at the rover. Frozen RPP regression (smoke/ekf/corner_pivot/segment_stop) still green with the overlay OFF. |
| **E** — Speed-proportional flow | `spray_flow_model.py` | ✅ **DONE @`d2b1b9e`** — `FlowModulator` (linear speed→flow, slew-limited, floor-seeded on ON edge, saturates at rated); `_update_flow` recomputes per tick from measured dt; ON command uses it; point mode fixed `point_dwell_flow_value`; manual not modulated; `commanded_flow_value`/`flow_source` telemetry. Flow updates ride the FSM-bypassing reassert → dropped flow ack never drops spray. 6 pure + 5 wiring tests. | ✅ **DONE (default OFF, mavlink_actuator only) — bench-verified params live + full-flow preserved; field-unverified. Needs the §7.5 3-speed calibration before enabling.** |
...
**Deliverables:** subscribe `/mavros/gpsstatus/gps1/raw`; param `spray_min_fix_type` (default 6 = RTK_FIXED); `gps_fix_timeout_s` (default 0.5, aligned with the drive/pose safety gates); **asymmetric hysteresis** — fail instantly on bad-or-stale, re-open only after `gps_recover_hold_s` (1.0 s) continuously good. Populate the existing `gps_fix_ok` / `gps_fix_name` status fields. Staleness is a *distinct* reason string from bad fix.
