...
| **D** — Point | Node FSM + coordinate ingest + **planner-side hold contract** | ◐ **NODE HALF DONE @`9218de4`** — `PointMeter` FSM (arrive/settle/dwell/OFF-confirm-before-advance/watchdog-skip), node routing, mode-appropriate config gate, Rev 4 §7.3 pivot-gate exemption, `mode_state`; server builder emits point; 9 pure + 3 wiring + 2 builder tests. Bench-verified live (node switches to point, builds meter, reports mode_state). | ◐ **PARTIAL — spray-node half done, bench-verified. PLANNER HALF NOT BUILT:** making the rover physically stop+hold on each dot (final-segment stop machinery) needs an RPP A/B or a mission-sequencing layer; point coords aren't extracted from a planned mission yet. Without it the rover drives through and the watchdog skips every point. |
