...
- **Lookahead + speed defaults RETUNED 2026-08-01 (field A/B, stg_30dd5bf8 + stg_a456f3a7):** `min_lookahead_dist=0.35`, `lookahead_time=1.0`, `mission_speed=0.5`. Ld is now genuinely speed-scaled (≈0.35 in the entry accel ramp — converges the post-pivot offset inside the 0.5 m pre-extension — v·1.0 at cruise). Line RMS 0.66–0.98 cm at 0.6 m/s vs 1.2–1.9 with the old constant 0.52/1.6. Arc curvature floor (Ld ≥ 0.35/k) untouched — curve @0.6 RMS 1.56, no regression (n=1). Validated band 0.35–0.6; **0.8 rung untested** (accel needs 0.91 m > extension). Defaults live in `src/rpp_controller_node.py` AND `server/routes/rpp_params.py` — the server registry pushes its values on profile apply, so change BOTH or the app silently reverts the node. `EKF2_WENC_CTRL=0` since 2026-08-01 evening (QGC-side).
