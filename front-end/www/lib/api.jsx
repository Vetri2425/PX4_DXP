...
let _baseUrl = localStorage.getItem("rover_base_url") || "http://localhost:5001";
function _getBaseUrl() { return _baseUrl; }
function _getSocketUrl() { return _baseUrl; }
...
const api = {
  ...
  getBaseUrl() { return _getBaseUrl(); },
  setBaseUrl(url) {
    if (!url) return;
    // Normalize: ensure http:// prefix, strip trailing slash
    let u = url.trim();
    if (!u.startsWith("http://") && !u.startsWith("https://")) u = "http://" + u;
    if (u.endsWith("/")) u = u.slice(0, -1);
    // Only change if different
    if (u === _baseUrl) return u;
    const prev = _baseUrl;
    _baseUrl = u;
    localStorage.setItem("rover_base_url", u);
  ...
  async discover() {
    try {
      const res = await fetch(`${_getBaseUrl()}/api/discover`, {
        method: "POST",
        headers: _headers(),
      });
      ...
      console.warn("[api] discover failed:", e.message);
      return [];
    }
  },
